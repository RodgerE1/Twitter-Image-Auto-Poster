...
def main():
    ...
    window = QWidget()
    window.setWindowTitle("Twitter Image Auto-Poster")  # Set the window title here

    window.setWindowIcon(
        QIcon("D:\\Documents\GitHub\\Twitter Image Auto-Poster\\x-Logo1.ico")
    )

    layout = QGridLayout()  # Change to QGridLayout for a grid arrangement
    ...
    tweet_sent_time_label = QLabel("")  # Separate label for time

    next_tweet_label = QLabel("Next Tweet At:")
    next_tweet_time_label = QLabel("")  # Separate label for next tweet time
